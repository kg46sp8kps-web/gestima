...
import { ref } from 'vue'
import { getIdoData } from '@/api/infor'
...
<template>
  ...
  <div class="section">
    ...
    <div class="toolbar">
      <button @click="loadInforData" :disabled="localLoading || !isConnected" class="btn-secondary">
        <Search :size="ICON_SIZE" /> {{ localLoading ? 'Načítám...' : 'Načíst dokumenty' }}
      </button>
      <button @click="emit('stage-all')" :disabled="inforDataCount === 0 || loading || localLoading" class="btn-secondary">
        <Download :size="ICON_SIZE" /> Napárovat na díly ({{ inforDataCount }})
      </button>
    </div>
