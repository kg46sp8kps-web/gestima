...
import { getIdoInfo as getInforIdoInfo, getIdoData as getInforIdoData } from '@/api/infor'
...
async function fetchFieldsForIdo() {
  if (!selectedIdo.value) {
    await alert({ title: 'Chyba validace', message: 'Please enter IDO name first' })
    return
  }
  ...
  try {
    ...
    await alert({ title: 'Chyba', message: 'Failed to fetch fields: ' + (err.response?.data?.detail || err.message) })
  } finally {
...
async function browseIdo() {
  if (!selectedIdo.value || !idoProperties.value) {
    await alert({ title: 'Chyba validace', message: 'Please select IDO and specify properties' })
    return
  }
