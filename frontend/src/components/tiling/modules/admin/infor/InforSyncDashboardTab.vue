...
import { getStatus, getLogs, triggerStep, updateStep, startSync, stopSync } from '@/api/infor-sync'
...
async function handleTrigger(stepName: string) {
  ...
  try {
    const result = await triggerStep(stepName)
...
async function handleToggleEnabled(step: SyncState) {
  try {
    await updateStep(step.step_name, { enabled: !step.enabled })
