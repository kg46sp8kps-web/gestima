...
import type { FtPartSummary, FtPartOperation } from '@/api/ft-debug'
...
// Inline detail — replaces window store navigation
...
function openDetail(partId: number) {
  selectedPartId.value = selectedPartId.value === partId ? null : partId
}
  ...
  <div class="ft-debug-tab">
    <div class="toolbar">
      ...
      <button @click="loadParts" :disabled="loading" class="btn-secondary" data-testid="ft-load-btn">
        <Search :size="ICON_SIZE" /> {{ loading ? 'Nacitam...' : 'Nacist data' }}
      </button>
      <button @click="precizCut = !precizCut"
              :class="['btn-secondary', precizCut ? 'btn-preciz-active' : 'btn-secondary']"
              title="Manning ≤100%, LATHE ≥50%, MILL ≥70%, ratio ≤5×"
              data-testid="ft-preciz-cut">
      ...
      <button @click="selectAllFiltered" class="btn-secondary" data-testid="ft-select-all">
        <Check :size="ICON_SIZE" /> Vybrat filtr.
      </button>
      <button @click="deselectAll" class="btn-secondary" data-testid="ft-deselect-all">
        <X :size="ICON_SIZE" /> Zrusit vyber
      </button>
      <button @click="exportJsonl" :disabled="selectedCount === 0 || loading" class="btn-primary" data-testid="ft-export-btn">
        <Download :size="ICON_SIZE" /> Export JSONL ({{ selectedCount }})
      </button>
    </div>
        ...
        <tbody :style="{ height: totalHeight + 'px', position: 'relative' }">
          ...
          <template v-for="part in visibleRows" :key="part.part_id">
            ...
            <tr :class="['part-row', { 'row-skipped': !part.is_eligible, 'row-expanded': expandedId === part.part_id }]"
              ...
              <td>
                <button v-if="part.file_id" @click.stop="openDetail(part.part_id)" class="btn-icon" data-testid="ft-open-detail">
    ...
    <!-- Inline detail panel — zobrazí se pod tabulkou při kliknutí na ikonu výkresu -->
    <div v-if="selectedPartId !== null" class="inline-detail">
      <FtPartDetail
        :part="parts.find(p => p.part_id === selectedPartId)!"
        @close="selectedPartId = null"
      />
    </div>

    <div v-else-if="!loading" class="empty-state">
      <Brain :size="ICON_SIZE_LG" />
...
<style scoped>
.ft-debug-tab { padding: 12px; overflow: auto; display: flex; flex-direction: column; gap: var(--pad); height: 100%; }
.toolbar { display: flex; gap: 6px; align-items: flex-end; flex-wrap: wrap; }
.spacer { flex: 1; }
.input-sm { width: 80px; }
.input-md { width: 130px; }
.stats-bar { display: flex; gap: 12px; font-size: var(--fs); color: var(--t3); padding: 6px var(--pad); background: var(--surface); border-radius: var(--r); border: 1px solid var(--b2); }
.stat-eligible { color: var(--ok); font-weight: 600; }
.stat-skipped { color: var(--t3); }
.stat-selected { color: var(--red); font-weight: 600; }
.table-scroll { overflow: auto; border: 1px solid var(--b2); border-radius: var(--r); flex: 1; min-height: 0; max-height: 520px; }
.ft-table { width: 100%; border-collapse: collapse; font-size: var(--fs); }
.ft-table th { background: var(--surface); padding: 4px 6px; text-align: left; font-weight: 600; color: var(--t3); border-bottom: 1px solid var(--b2); position: sticky; top: 0; z-index: 1; white-space: nowrap; }
.ft-table td { padding: 4px 6px; border-bottom: 1px solid var(--b1); white-space: nowrap; }
.ft-table tbody tr.part-row:hover { background: var(--b1); }
...
.btn-preciz-active { color: var(--ok); border-color: var(--ok); background: var(--green-10); font-weight: 600; }
.filter-sep { color: var(--t3); align-self: center; padding: 0 4px; }
.row-skipped { opacity: 0.5; }
.row-expanded { background: var(--raised); }
.clickable { cursor: pointer; color: var(--red); }
...
.badge-ok { display: inline-block; padding: 1px 6px; background: var(--green-10); color: var(--ok); border-radius: var(--rs); font-size: var(--fsm); font-weight: 600; }
.badge-skip { display: inline-block; padding: 1px 6px; background: var(--red-10); color: var(--err); border-radius: var(--rs); font-size: var(--fsm); font-weight: 600; max-width: 120px; overflow: hidden; text-overflow: ellipsis; }
.detail-row td { padding: 0; background: var(--raised); }
.inline-detail { border: 1px solid var(--b2); border-radius: var(--r); background: var(--raised); overflow: hidden; }
</style>
