 ...
 *
 * Import UI for Infor → Gestima materials (resizable two-panel layout)
 */
...
async function fetchFields() {
  ...
  try {
    ...
    await alert({ title: 'Error', message: err.response?.data?.detail || err.message || 'Unknown error' })
  } finally {
...
async function loadInforData() {
  if (!selectedIdo.value || selectedFields.value.length === 0) {
    await alert({ title: 'Chyba', message: 'Vyberte IDO a fieldy' })
    return
  }
...
async function stageSelected() {
  if (selectedRows.value.length === 0) {
    await alert({ title: 'Chyba', message: 'Vyberte řádky' })
    return
  }
...
async function openTestModal() {
  ...
  if (!testRow) {
    await alert({ title: 'Chyba', message: 'Není k dispozici žádný řádek pro test' })
    return
  }
...
async function executeImport() {
  if (selectedStagedRows.value.length === 0) {
    await alert({ title: 'Chyba', message: 'Vyberte řádky k importu' })
    return
  }
  ...
  if (validRows.length === 0) {
    await alert({ title: 'Chyba', message: 'Žádný vybraný řádek nemá platná data k importu' })
    return
  }
          ...
          <div class="table-header">
            ...
            <div class="selection-controls">
              ...
              <button @click="openTestModal" :disabled="testingPattern || stagedRows.length === 0" class="icon-btn" title="Test Pattern">
                <Settings :size="ICON_SIZE" v-if="!testingPattern" />
                <span v-else class="spinner"></span>
              </button>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="status-cell">
                    ...
                    <AlertTriangle v-else-if="getStatusIcon(row) === 'warning'" :size="ICON_SIZE" class="status-icon-warning" />
                    <XCircle v-else :size="ICON_SIZE" class="status-icon-error" />
                  </td>
    ...
    <div v-if="showTestModal && testResult" class="modal-overlay" @click="closeTestModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3><Settings :size="ICON_SIZE" /> Pattern Test Result</h3>
        ...
        <div class="modal-body">
          ...
          <!-- Parsed Results -->
          <div class="test-section">
            <label><CheckCircle :size="ICON_SIZE_SM" /> Parsed Results:</label>
          ...
          <!-- Detected Results -->
          <div class="test-section">
            <label><Search :size="ICON_SIZE_SM" /> Auto-Detected:</label>
            <div class="result-grid">
          ...
          <div v-if="testResultWarnings && testResultWarnings.length > 0" class="test-section">
            <label><AlertTriangle :size="ICON_SIZE_SM" /> Warnings:</label>
