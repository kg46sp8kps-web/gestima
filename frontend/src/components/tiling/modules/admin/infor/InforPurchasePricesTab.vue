...
<template>
  ...
  <div class="root">
    <div class="toolbar">
      ...
      <button class="btn-secondary" @click="analyze" :disabled="loading || !isConnected">Analyzovat</button>
      <button class="btn-secondary" @click="applySelected" :disabled="selected.size === 0">
        Aplikovat vybranych ({{ selected.size }})
      </button>
      <button class="btn-secondary" @click="refreshCache" :disabled="loading || !isConnected"><RefreshCw :size="ICON_SIZE" /></button>
    ...
    <div class="legend">
      ...
      <AlertTriangle :size="ICON_SIZE_SM" class="warning-icon" /><span>= malo dat (&lt;3 PO)</span>
    </div>
    ...
    <div v-else-if="analysis" class="results">
              ...
              <tr :class="{ 'row-selected': selected.has(cat.price_category_id) }">
                ...
                <td v-for="tier in cat.tiers.slice(0, 3)" :key="tier.tier_id" class="tier-cell">
                  ...
                  <div class="tier-content">
                    ...
                    <AlertTriangle v-if="!tier.sufficient_data" :size="ICON_SIZE_SM" class="warning-icon" title="Malo dat (<3 PO radku)" />
                  </div>
      ...
      <div v-if="analysis.unmatched.length > 0" class="unmatched-section">
        <button class="unmatched-toggle" @click="showUnmatched = !showUnmatched">
          <component :is="showUnmatched ? ChevronDown : ChevronRight" :size="ICON_SIZE" />
    ...
    <div v-else class="empty-state">
      <Download :size="ICON_SIZE_LG" class="empty-icon" />
...
.card-value { font-size: 16px; font-variant-numeric: tabular-nums; color: var(--t1); }
...
.tier-label-row { font-size: var(--fs); color: var(--t3); font-variant-numeric: tabular-nums; margin-bottom: 1px; }
.tier-content { display: flex; align-items: center; gap: 4px; font-size: var(--fs); font-variant-numeric: tabular-nums; }
...
.ui-code { font-variant-numeric: tabular-nums; color: var(--t1); }
.ui-desc, .text-secondary { color: var(--t3); }
.ui-reason { color: var(--warn); }
.ui-cost { font-variant-numeric: tabular-nums; color: var(--t1); text-align: right; }
