...
/**
 * TileTimeVision — AI/TimeVision modul pro tiling workspace
 * Zobrazuje TV estimaci + ML kalibraci pro fokusovaný díl.
 */
...
const focusedPart = computed(() => partsStore.getFocusedPart(props.ctx))
const partId = computed(() => focusedPart.value?.id ?? null)
const operations = computed(() => partId.value != null ? opsStore.forPart(partId.value) : [])
...
const hasAIEstimation = computed(() =>
  operations.value.some(op => op.ai_estimation_id != null),
)

const firstAIEstimationId = computed(() => {
  const op = operations.value.find(o => o.ai_estimation_id != null)
...
const tvBreakdown = computed<OperationBreakdown[]>(() => {
  ...
  try {
    return JSON.parse(tvEstimation.value.estimation_breakdown_json) as OperationBreakdown[]
  } catch {
    return []
  }
})
...
<template>
  <div class="tile-tv" data-testid="tile-time-vision">
    <!-- TimeVision Result -->
    <div v-if="tvEstimation || tvLoading" class="ai-section">
      <div class="section-header" @click="tvCollapsed = !tvCollapsed" data-testid="tv-section-toggle">
        <component :is="tvCollapsed ? ChevronRight : ChevronDown" :size="ICON_SIZE_SM" />
        <Sparkles :size="ICON_SIZE_SM" class="section-icon tv" />
        ...
        <template v-if="tvEstimation">
          <span class="status-badge" :style="{ color: getStatusColor(tvEstimation.status) }">
            <CheckCircle v-if="tvEstimation.status === 'verified'" :size="ICON_SIZE_SM" />
            <Clock v-else :size="ICON_SIZE_SM" />
    ...
    <div v-if="hasAIEstimation" class="ai-section">
      <div class="section-header" @click="mlCollapsed = !mlCollapsed" data-testid="ml-section-toggle">
        <component :is="mlCollapsed ? ChevronRight : ChevronDown" :size="ICON_SIZE_SM" />
        <GraduationCap :size="ICON_SIZE_SM" class="section-icon ml" />
        ...
        <span v-if="aiEstimation" class="status-badge" :style="{ color: getStatusColor(aiEstimation.status) }">
          <CheckCircle v-if="aiEstimation.status === 'verified'" :size="ICON_SIZE_SM" />
          <Clock v-else :size="ICON_SIZE_SM" />
          ...
          <div class="ml-body">
            <div class="ml-times">
              <div class="ml-col teal">
                ...
                <div class="ml-wrap">
                  <InlineInput :model-value="aiEstimation.estimated_time_min?.toFixed(1) ?? null" disabled class="ml-input" data-testid="ml-ai-time" />
              ...
              <div class="ml-col blue">
                ...
                <div class="ml-wrap">
                  <InlineInput
                    :model-value="mlHumanEstimate"
                    numeric
                    step="0.1"
                    min="0"
                    placeholder="—"
                    class="ml-input"
                    data-testid="ml-human-estimate"
                    @update:model-value="v => mlHumanEstimate = typeof v === 'number' ? v : null"
                  />
                  <span class="ml-unit">min</span>
                </div>
              </div>
              <div class="ml-col amber">
                ...
                <div class="ml-wrap">
                  <InlineInput
                    :model-value="mlActualTime"
                    numeric
                    step="0.1"
                    min="0"
                    placeholder="—"
                    class="ml-input"
                    data-testid="ml-actual-time"
                    @update:model-value="v => mlActualTime = typeof v === 'number' ? v : null"
                  />
                  <span class="ml-unit">min</span>
                </div>
              </div>
              <div class="ml-col grey">
                ...
                <div class="ml-wrap">
                  <InlineInput :model-value="currentOperationTime?.toFixed(1) ?? null" disabled class="ml-input" data-testid="ml-op-time" />
                  <span class="ml-unit">min</span>
                </div>
              </div>
            </div>
            ...
            <div class="ml-actions">
              <InlineSelect
                :model-value="mlPartType ?? ''"
                class="ml-select"
                title="Typ dílu"
                data-testid="ml-part-type"
                @update:model-value="v => mlPartType = (v as 'ROT' | 'PRI' | 'COMBINED') || null"
              >
                <option value="">—</option>
              ...
              <InlineSelect
                :model-value="mlComplexity ?? ''"
                class="ml-select"
                title="Složitost"
                data-testid="ml-complexity"
                @update:model-value="v => mlComplexity = (v as 'simple' | 'medium' | 'complex') || null"
              >
                <option value="">—</option>
              ...
              <InlineInput
                ...
                class="ml-notes"
                data-testid="ml-notes"
                @update:model-value="v => mlNotes = String(v ?? '')"
              />
    ...
    <!-- Empty state -->
    <div v-if="!tvEstimation && !tvLoading && !hasAIEstimation && partId" class="empty-state">
      <p>Žádná AI data pro tento díl</p>
    </div>

    <!-- No part focused -->
    <div v-if="!partId" class="empty-state">
      <p>Vyberte díl pro zobrazení AI odhadu</p>
    </div>
  </div>
</template>
