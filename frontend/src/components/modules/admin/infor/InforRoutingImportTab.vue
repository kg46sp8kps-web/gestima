...
/**
 * Routing Import Tab - Import Operations from Infor SLJobRoutes
 *
 * Default properties pre-filled: OperNum,DerJobItem,Wc,JshSchedHrs,JshSetupHrs,DerRunLbrHrs,DerRunMchHrs
...
const ROW_HEIGHT = 36  // px per staging row
const VISIBLE_ROWS = 60  // render buffer
...
// Staging state — use Set for O(1) selection checks with 200k+ rows
const stagedRows = ref<StagedRoutingRow[]>([])
...
// Counts (cached via computed, not re-filtered on every render)
...
async function stageAll() {
  ...
  try {
    const result = await previewRoutingImport(
      inforData.value,
      (done, total) => { progressDone.value = done; progressTotal.value = total }
    )
    // Strip infor_data to save memory (210k × 7 fields = heavy)
...
async function executeImport() {
  ...
  try {
    const result = await executeRoutingImport(
      rows,
      (done, total) => { progressDone.value = done; progressTotal.value = total }
    )
        ...
        <table class="staging-table">
          <thead>
            <tr>
              ...
              <th>Díl</th>
              <th class="col-num">Seq</th>
              <th class="col-num">Čas (min/ks)</th>
              <th class="col-num">SchedHrs (min)</th>
              <th class="col-num">Obsluha %</th>
              <th class="col-num">Setup (min)</th>
              <th>WC Infor</th>
              <th>WC Gestima</th>
              <th>Chyby</th>
            </tr>
          ...
          <tbody>
            ...
            <tr v-for="row in visibleRows" :key="row.row_index"
              ...
              <td class="col-num">{{ row.mapped_data.seq }}</td>
              <td class="col-num">{{ fmt(row.mapped_data.operation_time_min) }}</td>
              <td class="col-num">{{ fmt(row.mapped_data.sched_time_min) }}</td>
              <td class="col-num">{{ fmt(row.mapped_data.manning_coefficient) }}%</td>
              <td class="col-num">{{ fmt(row.mapped_data.setup_time_min) }}</td>
              <td>{{ row.mapped_data.infor_wc_code || '-' }}</td>
              <td>{{ row.mapped_data.work_center_id || '-' }}</td>
              <td class="errors-cell">{{ row.validation.errors.join(', ') || '-' }}</td>
            </tr>
            <!-- Spacer bottom -->
            <tr v-if="endIndex < stagedRows.length" :style="{ height: (stagedRows.length - endIndex) * ROW_HEIGHT + 'px' }"><td colspan="11"></td></tr>
          </tbody>
        </table>
      </div>

      <button @click="executeImport" :disabled="selectedValidCount === 0 || importing" class="btn-ghost btn-primary import-btn">
        <Download :size="ICON_SIZE.STANDARD" /> Importovat {{ selectedValidCount.toLocaleString() }} operací
      </button>
    </div>
  </div>
</template>
