...
async def get_machines() -> List[Dict[str, Any]]:
    """Load active machines from DB (cached, thread-safe)"""
    ...
    async with _cache_lock:
        ...
        try:
                ...
                _machines_cache = [
                    {
                        ...
                        "type": m.type,
                        "subtype": m.subtype,
                        "hourly_rate": m.hourly_rate_operation,  # Use computed property (operation rate with tools)
            ...
            logger.error(f"Database error loading machines: {e}", exc_info=True)
