"""GESTIMA - Milling Estimation Model (Phase 1: ML Time Estimation)

Stores extracted geometry features from STEP files for milling (PRI) parts.
Used to collect ground truth data for ML-based machining time estimation.

Classification: rotational_score ≤ 0.6 → MillingEstimation
...
class MillingEstimation(Base):
    """
    Milling (PRI) part estimation record with 79 extracted geometry features.
    ...
    """
    __tablename__ = "milling_estimations"
    ...
    part_type = Column(String(50), nullable=False, default="PRI")
    ...
    corrected_stock_type = Column(String(20), nullable=True)  # "cylinder", "box", "tube"
    corrected_stock_diameter = Column(Float, nullable=True)  # For cylinder/tube stock (mm)
    corrected_stock_length = Column(Float, nullable=True)  # Stock length (mm)
    correction_notes = Column(String(500), nullable=True)
    validated_by_user_id = Column(Integer, nullable=True)
    validation_date = Column(DateTime, nullable=True)

    # ========== VISION METADATA (AI Vision decision tracking) ==========
    data_source = Column(String(30), nullable=True)  # "vision_override", "occt_validated", "occt_fallback", "occt_only"
    confidence = Column(Float, nullable=True)  # Vision confidence 0-1 (or OCCT confidence)
    needs_manual_review = Column(Boolean, nullable=True, default=False)  # Flag for user review
    decision_log = Column(Text, nullable=True)  # JSON log of decision process
    ...
    def __repr__(self) -> str:
        return f"<MillingEstimation(id={self.id}, filename='{self.filename}', rotational_score={self.rotational_score:.2f})>"
