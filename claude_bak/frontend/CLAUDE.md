...
| **PartCombobox.vue** | Autocomplete výběr dílu | `modelValue`, `parts` |
| **WcCombobox.vue** | Autocomplete výběr pracoviště | `modelValue`, `workCenters` |
...
| **useSplitResize()** | Split-panel resize logika | resize handlers |
| **useItemTypeGuard()** | Type guard pro catalog items | type guard functions |
...
| `formatDuration(value)` | Duration format | `2h 15min` |
| `formatMinSec(value)` | Minutes:seconds | `3:45` |
| `formatPercent(value)` | Percent format | `85,3 %` |
| `partStatusLabel(status)` | Status → Czech label | `active` → `Aktivní` |
| `partSourceLabel(source)` | Source → Czech label | `manual` → `Manuální` |
| `quoteStatusLabel(status)` | Quote status → Czech label | `draft` → `Koncept` |
